// ==UserScript==
// @name           Macros Modified (h for help)
// @namespace      http://groups-beta.google.com/group/gmail-power-users
// @description	   Adds extra, customizable keyboard shortcuts and macros for Gmail 2.0.  Press h for help.
// @include        http://mail.google.com/*
// @include        https://mail.google.com/*

// @homepage http://userscripts.org/scripts/show/14189
// @author Mihai Parparita, modified by Brent Nef
// @tab General
// ==/UserScript==

// Auto-update code
var scriptVersion=20090206.1;
...
//  clickMoreActionsMenuItem(action.label, action.act);
   if (action.label)
      return clickMoreActionsMenuItem( /^more actions/i, 
                                       MORE_ACTIONS_MENU_ITEM_CLASS, 
                                       new RegExp(action.label, "i"));
   else
      debug("mmmm..."+action.label);
   return false;
}

function doButton(action) {
  var button = getFirstVisibleNode(
      evalXPath(".//div[@act='" + action.act + "']", getDoc().body));

  if (button) {
    simulateClick(button, "mousedown");
    simulateClick(button, "mouseup");
    debug("Simulated Click for doButton: "+action.label+": \n"
          + button.innerHTML);
//    simulateClick(getDoc().body,"click");
    return true;
  }
  else
     debug("What no action? "+action.label);
...
var MORE_ACTIONS_MENU_ITEM_CLASS = "goog-menuitem";

var LABEL_ITEM_CLASS_NAME = "zD5BAe";
var QUICK_LINKS_DIV_CLASS = "KnF38d";
var QUICK_LINKS_A_CLASS   = "aUMFHd";
...
//      clickMoreActionsMenuItem(labelName, ADD_LABEL_ACTION.act);
      return clickMoreActionsMenuItem(/Labels/i, "goog-tristatemenuitem-content", new RegExp(labelName,"i"));
    },
  },
  'REMOVELABEL': {
    cat: 'LABELS',
...
//        cmami(ARCHIVE_ACTION);
  ...
  'READARCHIVE': { 
    ...
    func: function() {
      if (!ACTIONS['READ'].func()) {
         debug("Problem with READ function");
         return;
      }
...
//        cmami(TRASH_ACTION);
  ...
  'READ': {
    ...
    desc: 'Mark as read',
    view: 'tl',
    func: function() { return cmami(MARK_AS_READ_ACTION); }
  },
  'READTRASH': {
    cat: 'THREADLIST',
...
function simulateClick(node, eventType) {
  ...
  if (node.dispatchEvent(event))
     debug("dispatched event..."+eventType+" \n"+node.innerHTML);
  else
     debug("somebody used a preventdefault");
}

function clickMoreActionsMenuItem(menuRE, menuItemClass, menuItemRE) {
  
   var menu; 
   var return_val = false;
   var possibleMenu = getNodesByTagNameAndClass(
       getDoc().body, "div", "goog-imageless-button-content");
   for (var i = possibleMenu.length-1; i >= 0; --i){
//      debug("i: "+i+" -- length: "+possibleMenu.length); 
//      debug("i: "+i+" -- >> "+possibleMenu[i].innerHTML); 

      if (menuRE.test(possibleMenu[i].innerHTML)){
         debug("FOUND IT");
         menu = possibleMenu[i];
         simulateClick(menu, "mousedown");
         var menuItemNodes = getNodesByTagNameAndClass(getDoc().body, "div", menuItemClass);

         if (!menuItemNodes)
            debug("Couldn't find any menu items...");

         var menuItemNode;
         for (var j=menuItemNodes.length-1; j >= 0; --j){
            var tmpNode = menuItemNodes[j];

            // offsetheight is used to check if it's visible
            if (menuItemRE.test(tmpNode.innerHTML) ){
               debug("Found match with tmpNode: "+tmpNode.innerHTML);
               if (tmpNode.offsetHeight){
                  menuItemNode = tmpNode;
                  break;
               }
               else debug("tmpNode is not visible");
            }
         }

         if (!menuItemNode) {
            debug("Couldn't find menu item");
         } else {
  ...
  if (!menu) {
    debug("Couldn't find the menu header node");
  }
...
//    var quickLinksNode = getNodesByTagNameAndClass(navPaneNode, "div", QUICK_LINKS_DIV_CLASS);
    var quickLinks = getNodesByTagNameAndClass(navPaneNode, 'a', QUICK_LINKS_A_CLASS);
    if(quickLinks){
