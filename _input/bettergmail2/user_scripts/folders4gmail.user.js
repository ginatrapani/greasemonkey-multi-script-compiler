...
// @homepage     http://www.arend-von-reinersdorff.com/folders4gmail/
// @description   Lists labels in a folder-like hierarchy.
// @include       http://mail.google.com/*
// @include       https://mail.google.com/*

// @author Arend v. Reinersdorff
// @tab Sidebar
// @versionorlastupdate 1.52 2009-07-09, Label update timeout set to 5000ms by G. Trapani
 ...
 * Folders4Gmail uses the jQuery library, which is licensed under an MIT license.
 */
...
//folders4gmail for ui=2
function ui2(){

var canvasFrame     = $("#canvas_frame").contents();
var lastLabelInView = "";
var labelMap        = new mapWithCount();
var rlmChar         = ($("body").attr("dir") == "rtl") ? "\u200f" : "";
var unreadNameRegex = new RegExp("^(.+) " + rlmChar + "\\((\\d+)\\)" + rlmChar + "$");
var $labelParent;


function mapWithCount(){
...
//prevent problem with IE script loading
if($("#f4g_style", canvasFrame).length){
    return;
}
...
function checkForLabelUpdates(){
    ...
    if(labelMap.size() > $labelNodes.length){
        $labelNodes.addClass("f4g_isInDom");
        applyToTopLabels(checkDeleted);
    ...
    if(labelMap.size() > $labelParent.find("span.f4g_countWatch").length){
        applyToTopLabels(checkUnreadCount);
    }
...
function applyToTopLabels(aFunction){
    for(var canonicalLabelName in labelMap.values){
        var label = labelMap.values[canonicalLabelName];
...
function addLabel(){
    ...
    if(existingLabel){
        ...
        var parentLimit         = label.canonicalName.lastIndexOf("/");
        var canonicalParentName = label.canonicalName.substring(0, parentLimit);

        if(canonicalParentName){
            parentLabel = labelMap.get(canonicalParentName);
            if(!parentLabel){
                var parentLabelName = label.fullName.substring(0, parentLimit);
                createFakeLabel(label.$node, parentLabelName);
            }
            parentLabel = labelMap.get(canonicalParentName);
        }
        ...
        if(parentLabel){
            ...
            parentLabel.childLabels.add(label.canonicalName, label);

            setExpanderDisplay(parentLabel);
        }

        labelMap.add(label.canonicalName, label);
    }

    checkFakeParentLocation(label);
    setLabelDisplay(label);
    addExpanderAndIndent(label);
}
...
function checkFakeParentLocation(label){
    ...
    if(!parentLabel.$node.hasClass("f4g_fake")){
        return;
    }

    var $firstChildNode = getFirstChildNode(parentLabel);
    if(parentLabel.$node.get(0) != $firstChildNode.get(0)){
        $firstChildNode.before(parentLabel.$node);
        setLabelDisplay(parentLabel);

        checkFakeParentLocation(parentLabel);
    }
}

//get the first dom $node in the document
//that is either the dom $node of the label or the dom $node of a child label
function getFirstChildNode(label){
    var $nodes = label.$node;

    for(var canonicalLabelName in label.childLabels.values){
        var childLabel = label.childLabels.get(canonicalLabelName);
        $nodes = $nodes.add(childLabel.$node);
    }

    $nodes.addClass("f4g_test");
    var $orderedNodes = $labelParent.find("div.f4g_test");
    $nodes.removeClass("f4g_test");
    return $orderedNodes.eq(0);
}


//label object
function Label($labelNode){
    this.childLabels = new mapWithCount();

    this.initIsExpanded = function(){
        this.isExpanded = false;
    }
    this.initIsExpanded();

    this.reset = function($labelNode){
        this.$node = $labelNode;
        ...
        if($labelNode.find("div.n1").length){
            ...
            if(regexResult){
                fullName = regexResult[1];
            }
        }
        this.fullName = fullName;

        var   standardName = standardSeparator(fullName);
        var  canonicalName = standardName.toLowerCase();
        this.canonicalName = canonicalName;

        var shortName = standardName;
        var nameIndex = shortName.lastIndexOf("/") + 1;
        shortName     = shortName.substring(nameIndex);
        shortName     = shortName.replace(/^ *([^ ].*)$/, "$1");
        shortName     = shortName.replace(/^(.*[^ ]) *$/, "$1");
        //empty label name, make visible
        if(/^ *$/.test(shortName)){
            shortName = "\u00A0";
        }
        this.shortName = shortName;
    }
    this.reset($labelNode);
}
...
function createFakeLabel($labelNode, parentLabelName){
    ...
    //set right name
    $aNode.attr("title", parentLabelName);

    //prevent goto-label on click
    $aNode.click(function(e){ e.stopPropagation(); });
...
function setExpanderDisplay(label){
    ...
    if(label.isExpanded){
        ...
        $expanderNode.removeClass("pv");
        $expanderNode.addClass   ("pr");
    }

    var visibilityString = (label.childLabels.size()) ? "visible" : "hidden";
    $expanderNode.css("visibility", visibilityString);
}


function setLabelDisplay(label){
    var displayString = "";
    var parentLabel   = label.parentLabel;
    if(parentLabel){
        if(!parentLabel.isExpanded || parentLabel.$node.css("display") == "none"){
             displayString = "none";
        }
    }
    ...
    for(var canonicalLabelName in label.childLabels.values){
        setLabelDisplay(label.childLabels.get(canonicalLabelName));
    }
}
...
function checkDeleted(label){
    for(var canonicalChildLabelName in label.childLabels.values){
        var childLabel = label.childLabels.get(canonicalChildLabelName);
        checkDeleted(childLabel);
    }

    if(label.$node.hasClass("f4g_isInDom")){
        return;
    }

    if(!label.childLabels.size()){
        labelMap.remove(label.canonicalName);
        var parentLabel = label.parentLabel;
        if(parentLabel){
            parentLabel.childLabels.remove(label.canonicalName);
            if(!parentLabel.childLabels.size()){
                parentLabel.initIsExpanded();
                setExpanderDisplay(parentLabel);
            }
            parentLabel.$node.find("span.f4g_countWatch").remove();
        }
        return;
    }

    var $firstChildNode = getFirstChildNode(label);
    createFakeLabel($firstChildNode, label.fullName);
}


function checkUnreadCount(label){
    for(var canonicalChildLabelName in label.childLabels.values){
        var childLabel = label.childLabels.get(canonicalChildLabelName);
        checkUnreadCount(childLabel);
    }
    ...
    if(!$labelNode.find("span.f4g_countWatch").length){
        ...
        if($labelNode.find("div.n1").length){
            ...
            //there might be no unread count because of gmail labs 'hide unread counts' feature
            if(regexResult){
                label.normalUnreadCount = parseInt(regexResult[2]);
            }
        }
        ...
        for(var canonicalChildLabelName in label.childLabels.values){
            var childLabel = label.childLabels.get(canonicalChildLabelName);
            label.summedUnreadCount += childLabel.summedUnreadCount;
        }

        var unreadText = "";
        if(label.summedUnreadCount){
            unreadText = " " + rlmChar + "(" + label.summedUnreadCount + ")" + rlmChar;
        }
        var countWatchHtml = "<span class='f4g_countWatch'></span>";
        $aNode.html(label.shortName + unreadText + countWatchHtml);

        if(label.parentLabel){
            label.parentLabel.$node.find("span.f4g_countWatch").remove();
        }
    }
}


function expandLabelInView(){
    var locationHash = window.location.href.replace(/^[^#]+#label\/([^\/]+).*$/, "$1");
    locationHash = locationHash.replace(/\+/g, " ");
    locationHash = decodeURIComponent(locationHash);

    var canonicalLabelName = standardSeparator(locationHash).toLowerCase();

    var label = labelMap.get(canonicalLabelName);
    if((canonicalLabelName == lastLabelInView) || !label){
        return;
    }
    lastLabelInView = canonicalLabelName;
...
//folders4gmail for ui=1
function ui1(){

var uiHandler = new ui1Handler();
